...
function renderSubMenu(activeAccountId) {
  return (accountType, i) => {
    return (
      <SubMenu key={`sub${i}`} title={<span><Icon type={accountType.icon} /><span>{accountType.title}</span></span>}>
          {
            accountType.accounts.map((account) => {
              ...
              const accountLabel = isCurrentAccount ? account.name : <Link to={`/account/${account._id}`}>{account.name}</Link>;
              return (
...
const AccountsGroup = ({ accounts, activeAccountId }) => {
  if (!accounts) {
    return (
      <Row type="flex" justify="center" align="middle" style={{ height: '90%' }}>
  ...
  return (
    <Menu theme="dark" mode="inline" selectedKeys={[activeAccountId]} defaultOpenKeys={allSubMenus}>
